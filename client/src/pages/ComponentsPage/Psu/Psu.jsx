...
function Psu() {
  ...
  const showAllPsu = async (page) => {
    ...
    const response = await apiSubmit({
      url: apis().getAllPsu.url,
      method: apis().getAllPsu.method,
      query: { data: payload },
      successMessage: null,
  ...
  const handlePowerSupplyTypeFilter = (e) => {
    const isChecked = e.target.checked;
    if (isChecked) {
      setFilters((prev) => ({
        ...prev,
        powerSupplyType: e.target.value,
      }));
    } else {
      setFilters((prev) => ({
        ...prev,
  ...
  const handleEfficiencyRatingFilter = (e) => {
    const isChecked = e.target.checked;
    if (isChecked) {
      setFilters((prev) => ({
        ...prev,
  ...
  const handleModularTypeFilter = (e) => {
    const isChecked = e.target.checked;
    if (isChecked) {
      setFilters((prev) => ({
        ...prev,
        modularType: e.target.value,
      }));
    } else {
      setFilters((prev) => ({
        ...prev,
  ...
  useEffect(() => {
    showAllPsu(paginations.productCurrentPage);
  }, [
    paginations.productCurrentPage,
    filters?.manufacturor,
    filters?.price,
    filters?.featured,
    filters?.powerSupplyType,
    filters?.efficiencyRating,
    filters?.modularType,
  ]);
      ...
      <Container className={"w-full my-[3rem]"}>
        <div className="mx-auto max-w-7xl px-2 py-10 lg:px-10">
          <div className="md:flex md:flex-row md:items-start md:justify-between">
            <div>
              <h1 className="h1">Psu Products</h1>
            </div>
          </div>
          <hr className="my-8" />
          <div className="lg:grid lg:grid-cols-12 lg:gap-x-6">
            <div className="space-y-6 divide-y lg:col-span-3 lg:block">
              ...
              <div className="pt-6">
                ...
                <ul className="mt-2 ">
                  {["ATX", "Flex ATX", "Mini ITX", "SFX", "TFX"].map((item) => (
                    <li
                      key={item}
                      className="flex items-center justify-between "
                    >
                      <div className="flex items-center">
                        <Checkbox
                          sx={{ "&.Mui-checked": { color: "#3238f2" } }}
                          value={item}
                          onChange={handlePowerSupplyTypeFilter}
                        />
                        <label
                          htmlFor="color-white"
                          className="ml-3 text-sm font-medium text-gray-900"
                    ...
                    >
                      <div className="flex items-center">
                        <Checkbox
                          sx={{ "&.Mui-checked": { color: "#3238f2" } }}
                          value={item}
                          onChange={handleEfficiencyRatingFilter}
                        />
                        <label
                          htmlFor="color-white"
                          className="ml-3 text-sm font-medium text-gray-900"
              ...
              <div className="pt-6">
                ...
                <ul className="mt-2 ">
                  {["Full", "Semi", "Non-modular"].map((item) => (
                    <li
                      key={item}
                      className="flex items-center justify-between "
                    >
                      <div className="flex items-center">
                        <Checkbox
                          sx={{ "&.Mui-checked": { color: "#3238f2" } }}
                          value={item}
                          onChange={handleModularTypeFilter}
                        />
                        <label
                          htmlFor="color-white"
                          className="ml-3 text-sm font-medium text-gray-900"
