...
function Case() {
  ...
  const showAllCase = async (page) => {
    ...
    if (filters?.sidePanel) {
      payload.sidePanel = filters?.sidePanel;
    }

    if (filters?.frontPanel) {
      payload.frontPanel = filters?.frontPanel;
    }

    if (filters?.formFactor) {
      payload.formFactor = filters?.formFactor;
    }

    const response = await apiSubmit({
      url: apis().getAllCase.url,
      method: apis().getAllCase.method,
      query: { data: payload },
      successMessage: null,
  ...
  const handleFrontPanelFilter = (e) => {
    const isChecked = e.target.checked;
    if (isChecked) {
      setFilters((prev) => ({
        ...prev,
        frontPanel: e.target.value,
      }));
    } else {
      setFilters((prev) => ({
        ...prev,
  ...
  const handleFormFactorFilter = (e) => {
    const isChecked = e.target.checked;
    if (isChecked) {
      setFilters((prev) => ({
        ...prev,
  ...
  useEffect(() => {
    showAllCase(paginations.productCurrentPage);
  }, [
      ...
      <Container className={"w-full my-[3rem]"}>
        <div className="mx-auto max-w-7xl px-2 py-10 lg:px-10">
          <div className="md:flex md:flex-row md:items-start md:justify-between">
            <div>
              <h1 className="h1">Case Products</h1>
            </div>
          </div>
          <hr className="my-8" />
          <div className="lg:grid lg:grid-cols-12 lg:gap-x-6">
            <div className="space-y-6 divide-y lg:col-span-3 lg:block">
              ...
              <div className="pt-6">
                ...
                <ul className="mt-2 ">
                  {["Mesh", "Tinted", "Tempered Glass"].map((item) => (
                    <li
                      key={item}
                      className="flex items-center justify-between "
                    >
                      <div className="flex items-center">
                        <Checkbox
                          sx={{ "&.Mui-checked": { color: "#3238f2" } }}
                          value={item}
                          onChange={handleSidePanelFilter}
                        />
                        <label
                          htmlFor="color-white"
                          className="ml-3 text-sm font-medium text-gray-900"
                    ...
                    >
                      <div className="flex items-center">
                        <Checkbox
                          sx={{ "&.Mui-checked": { color: "#3238f2" } }}
                          value={item}
                          onChange={handleFrontPanelFilter}
                        />
                        <label
                          htmlFor="color-white"
                          className="ml-3 text-sm font-medium text-gray-900"
                    ...
                    >
                      <div className="flex items-center">
                        <Checkbox
                          sx={{ "&.Mui-checked": { color: "#3238f2" } }}
                          value={item}
                          onChange={handleFormFactorFilter}
                        />
                        <label
                          htmlFor="color-white"
                          className="ml-3 text-sm font-medium text-gray-900"
