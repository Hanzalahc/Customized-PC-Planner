...
function Cpu() {
  ...
  const showAllCpu = async (page) => {
    ...
    if (filters?.stockCooler) {
      payload.stockCooler = filters?.stockCooler;
    }

    const response = await apiSubmit({
      url: apis().getAllCpu.url,
      method: apis().getAllCpu.method,
      query: { data: payload },
      successMessage: null,
  ...
  const handleStockCoolerFilter = (e) => {
    const isChecked = e.target.checked;
    if (isChecked) {
      setFilters((prev) => ({
        ...prev,
        stockCooler: e.target.value,
      }));
    } else {
      setFilters((prev) => ({
        ...prev,
  ...
  useEffect(() => {
    showAllCpu(paginations.productCurrentPage);
  }, [
      ...
      <Container className={"w-full my-[3rem]"}>
        <div className="mx-auto max-w-7xl px-2 py-10 lg:px-10">
          <div className="md:flex md:flex-row md:items-start md:justify-between">
            <div>
              <h1 className="h1">Cpu Products</h1>
            </div>
          </div>
          <hr className="my-8" />
                  ...
                  <li className="flex items-center justify-between ">
                    <div className="flex items-center">
                      <Checkbox
                        sx={{ "&.Mui-checked": { color: "#3238f2" } }}
                        value={true}
                        onChange={handleStockCoolerFilter}
                      />
                      <label
                        htmlFor="color-white"
                        className="ml-3 text-sm font-medium text-gray-900"
