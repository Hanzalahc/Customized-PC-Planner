...
function Storage() {
  ...
  const showAllStorage = async (page) => {
    const payload = {
      pageNum: page,
    };
    ...
    if (filters?.storageType) {
      payload.storageType = filters?.storageType;
    }

    const response = await apiSubmit({
      url: apis().getAllStorage.url,
      method: apis().getAllStorage.method,
      query: { data: payload },
      successMessage: null,
    ...
    if (response?.success) {
      setStorageData(response?.data.storages);
      setPaginations({
  ...
  const handleStorageTypeFilter = (e) => {
    const isChecked = e.target.checked;
    if (isChecked) {
      setFilters((prev) => ({
        ...prev,
  ...
  useEffect(() => {
    showAllStorage(paginations.productCurrentPage);
  }, [
      ...
      <Container className={"w-full my-[3rem]"}>
        <div className="mx-auto max-w-7xl px-2 py-10 lg:px-10">
          <div className="md:flex md:flex-row md:items-start md:justify-between">
            <div>
              <h1 className="h1">Storage Products</h1>
            </div>
          </div>
          <hr className="my-8" />
                    ...
                    >
                      <div className="flex items-center">
                        <Checkbox
                          sx={{ "&.Mui-checked": { color: "#3238f2" } }}
                          value={item}
                          onChange={handleStorageTypeFilter}
                        />
                        <label
                          htmlFor="color-white"
                          className="ml-3 text-sm font-medium text-gray-900"
