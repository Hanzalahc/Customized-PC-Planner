...
import Motherboard from "../../models/componentsModel/Motherboard.js";
import { asyncHandler } from "../../utils/asyncHandler.js";
import { ApiError } from "../../utils/ApiError.js";
import Joi from "joi";
import { deleteImageFromCloudinary } from "../../utils/cloudinary.js";

export const AddMotherboard = asyncHandler(async (req, res, next) => {
  ...
  const { error } = validateMotherboard(req.body);
  if (error) {
  ...
  if (!savedMotherboard) {
    return next(
      new ApiError(500, "Something went wrong while saving Motherboard")
    );
  }
...
export const GetAllMotherboardWithFiltersAndPagination = asyncHandler(
  async (req, res, next) => {
    const pageNum =
    ...
    const allMotherboards = await Product.find(filters)
    ...
    if (!allMotherboards || allMotherboards.length === 0) {
      return next(new ApiError(404, "No Motherboards found"));
    }

    const totalMotherboards = await Product.countDocuments(filters);

    const totalPages = Math.ceil(totalMotherboards / pageSize);

    res.status(200).json({
      success: true,
      message: "All Motherboards fetched successfully",
      data: {
        motherboards: allMotherboards,
        totalPages: totalPages,
        totalItems: totalMotherboards,
      },
    });
  }
);

export const GetAllMotherboardDropdown = asyncHandler(
  async (req, res, next) => {
    const allMotherboards = await Product.find({
      status: "active",
      productType: "Motherboard",
    }).select(
      " images name description price oldPrice model discount rating socket memoryType formFactor"
    );
    ...
    const totalMotherboards = allMotherboards.length;
...
// export const GetSingleMotherboard = asyncHandler(async (req, res, next) => {
...
//   const findedMotherboard = await Product.findById(productId).populate([
//     {
//       path: "reviews",
...
//     data: findedMotherboard,
//   });
// });

export const UpdateMotherboard = asyncHandler(async (req, res, next) => {
  const userData = req.user;
  ...
  const isSameData =
    ...
    product.memorySlots === memorySlots &&
    product.pcieSlots === pcieSlots &&
    product.sataM2Slots === sataM2Slots;
  ...
  const updatedMotherboard = await Product.findByIdAndUpdate(
    productId,
    {
      name,
      description,
  ...
  if (!updatedMotherboard) {
    return next(
      new ApiError(500, "Something went wrong while updating Motherboard")
    );
  }
...
export const RemoveMotherboardById = asyncHandler(async (req, res, next) => {
  const userData = req.user;
...
function validateMotherboard(data) {
  const schema = Joi.object({
    ...
    memorySlots: Joi.number().required(),
    pcieSlots: Joi.number().required(),
    sataM2Slots: Joi.number().required(),
  });
