...
import Cooler from "../../models/componentsModel/Cooler.js";
import { asyncHandler } from "../../utils/asyncHandler.js";
import { ApiError } from "../../utils/ApiError.js";
import Joi from "joi";
import { deleteImageFromCloudinary } from "../../utils/cloudinary.js";

export const AddCooler = asyncHandler(async (req, res, next) => {
  ...
  const { error } = validateCooler(req.body);
  if (error) {
  ...
  if (!savedCooler) {
    return next(new ApiError(500, "Something went wrong while saving COOLER"));
  }

  res.status(201).json({
    message: "Cooler created successfully",
    success: true,
    data: savedCooler,
  });
});

export const GetAllCoolerWithFiltersAndPagination = asyncHandler(
  async (req, res, next) => {
    const pageNum =
    ...
    if (req.query.data.isWaterCooled) {
      filters.isWaterCooled =
        req.query.data.isWaterCooled === "true" ? true : false;
    }
    if (req.query.data.isFanless) {
      filters.isFanless = req.query.data.isFanless === "true" ? true : false;
    }
    const sortOrder = req.query.data?.price === "low" ? 1 : -1;

    const allCoolers = await Product.find(filters)
    ...
    if (!allCoolers || allCoolers.length === 0) {
      return next(new ApiError(404, "No COOLERs found"));
    }

    const totalCoolers = await Product.countDocuments(filters);

    const totalPages = Math.ceil(totalCoolers / pageSize);

    res.status(200).json({
      success: true,
      message: "All COOLERs fetched successfully",
      data: {
        coolers: allCoolers,
        totalPages: totalPages,
        totalItems: totalCoolers,
      },
    });
  }
);

export const GetAllCoolerDropdown = asyncHandler(async (req, res, next) => {
  const allCoolers = await Product.find({
    status: "active",
    productType: "Cooler",
  }).select(
    " images name description price oldPrice discount rating socket model"
  );
  ...
  const totalCoolers = allCoolers.length;
...
// export const GetSingleCooler = asyncHandler(async (req, res, next) => {
...
//   const findedCooler = await Product.findById(productId).populate([
//     {
//       path: "reviews",
...
export const UpdateCooler = asyncHandler(async (req, res, next) => {
  const userData = req.user;
  ...
  const isSameData =
    ...
    product.isWaterCooled === isWaterCooled &&
    product.isFanless === isFanless;
  ...
  const updatedCooler = await Product.findByIdAndUpdate(
    productId,
    {
      name,
      description,
  ...
  if (!updatedCooler) {
    return next(
      new ApiError(500, "Something went wrong while updating COOLER")
    );
  }
...
export const RemoveCoolerById = asyncHandler(async (req, res, next) => {
  const userData = req.user;
...
function validateCooler(data) {
  const schema = Joi.object({
    ...
    isWaterCooled: Joi.boolean().required(),
    isFanless: Joi.boolean().required(),
  });
