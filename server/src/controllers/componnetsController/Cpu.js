// import Category from "../../models/PreBuildCategory.js";
import Product from "../../models/Product.js";
import Cpu from "../../models/componentsModel/Cpu.js";
import { asyncHandler } from "../../utils/asyncHandler.js";
import { ApiError } from "../../utils/ApiError.js";
import Joi from "joi";
import { deleteImageFromCloudinary } from "../../utils/cloudinary.js";

export const AddCpu = asyncHandler(async (req, res, next) => {
  const userData = req?.user;

  if (userData?.role !== "admin") {
    return next(new ApiError(403, "Unauthorized request"));
  }

  const { error } = validateCpu(req.body);
  if (error) {
    return next(new ApiError(400, error.details[0].message));
  }

  const {
    name,
    description,
    content,
    images,
    brand,
    price,
    oldPrice,
    discount,
    stock,
    isFeatured,
    model,
    warranty,
    weight,
    powerCategory,
    productType,
    manufacturor,
    performanceCoreFrequency,
    socket,
    coreCount,
    threadCount,
    l2Cache,
    l3Cache,
    benchmark,
    stockCooler,
  } = req.body;

  const duplicateProduct = await Product.findOne({
    name,
    productType,
    model,
  });

  if (duplicateProduct) {
    return next(new ApiError(409, "Product already exists with this name"));
  }

  const newCpu = new Cpu({
    name,
    description,
    content,
    images,
    brand,
    price,
    oldPrice,
    discount,
    stock,
    isFeatured,
    model,
    warranty,
    weight: weight ?? "",
    powerCategory,
    productType,
    manufacturor,
    performanceCoreFrequency,
    socket,
    coreCount,
    threadCount,
    l2Cache: l2Cache ?? 0,
    l3Cache: l3Cache ?? 0,
    benchmark,
    stockCooler,
  });

  const savedCpu = await newCpu.save();

  if (!savedCpu) {
    return next(new ApiError(500, "Something went wrong while saving CPU"));
  }

  res.status(201).json({
    message: "Cpu created successfully",
    success: true,
    data: savedCpu,
  });
});

export const GetAllCpuWithFiltersAndPagination = asyncHandler(
  async (req, res, next) => {
    const pageNum =
      Number(req.query.data.pageNum) > 0 ? Number(req.query.pageNum) : 1;
    const pageSize = 10;

    const filters = {
      status: "active",
      productType: "Cpu",
    };

    if (req.query.data.manufacturor) {
      filters.manufacturor = req.query.data.manufacturor;
    }

    if (req.query.data.featured) {
      filters.isFeatured = req.query.data.featured;
    }

    if (req.query.data.featured === "no") {
      filters.isFeatured = false;
    }

    if (req.query.data.socket) {
      filters.socket = req.query.data.socket;
    }

    if (req.query.data.stockCooler) {
      filters.stockCooler = req.query.data.stockCooler;
    }

    if (req.query.data.stockCooler === "no") {
      filters.stockCooler = false;
    }

    const sortOrder = req.query.data?.price === "low" ? 1 : -1;

    const allCpus = await Product.find(filters)
      .sort(req.query.data?.price ? { price: sortOrder } : {}) 
      .select("images name description price oldPrice discount rating")
      .skip((pageNum - 1) * pageSize)
      .limit(pageSize);

    if (!allCpus || allCpus.length === 0) {
      return next(new ApiError(404, "No CPUs found"));
    }

    const totalCpus = await Product.countDocuments(filters);

    const totalPages = Math.ceil(totalCpus / pageSize);

    res.status(200).json({
      success: true,
      message: "All CPUs fetched successfully",
      data: {
        cpus: allCpus,
        totalPages: totalPages,
        totalItems: totalCpus,
      },
    });
  }
);

export const GetAllCpuDropdown = asyncHandler(async (req, res, next) => {
  const allCpus = await Product.find({
    status: "active",
    productType: "Cpu",
  }).select(
    " images name description price oldPrice discount rating socket model benchmark"
  );
  if (!allCpus || allCpus.length === 0) {
    return next(new ApiError(404, "No CPUs found"));
  }

  const totalCpus = allCpus.length;

  res.status(200).json({
    success: true,
    message: "All CPUs fetched successfully",
    data: {
      cpus: allCpus,
      totalCpus,
    },
  });
});

// export const GetSingleCpu = asyncHandler(async (req, res, next) => {
//   const { productId } = req.params;

//   if (!productId) {
//     return next(new ApiError(400, "Product id is required"));
//   }

//   const findedCpu = await Product.findById(productId).populate([
//     {
//       path: "reviews",
//       select: "rating review user createdAt",
//       populate: {
//         path: "user",
//         select: "name email avatar",
//       },
//     },
//   ]);

//   if (!findedCpu) {
//     return next(new ApiError(404, "Product not found"));
//   }

//   return res.status(200).json({
//     success: true,
//     message: "Product fetched successfully",
//     data: findedCpu,
//   });
// });

export const UpdateCpu = asyncHandler(async (req, res, next) => {
  const userData = req.user;

  if (userData.role !== "admin") {
    return next(new ApiError(403, "Unauthorized request"));
  }

  const productId = req.params.productId;

  const { error } = validateCpu(req.body);
  if (error) {
    return next(new ApiError(400, error.details[0].message));
  }

  if (!productId) {
    return next(new ApiError(400, "Product id is required"));
  }

  const product = await Product.findById(productId);

  if (!product) {
    return next(new ApiError(404, "Product not found with this ID"));
  }

  const {
    name,
    description,
    content,
    images,
    brand,
    price,
    oldPrice,
    discount,
    stock,
    status,
    isFeatured,
    model,
    warranty,
    weight,
    powerCategory,
    productType,
    manufacturor,
    performanceCoreFrequency,
    socket,
    coreCount,
    threadCount,
    l2Cache,
    l3Cache,
    benchmark,
    stockCooler,
  } = req.body;

  const isSameData =
    product.name === name &&
    product.description === description &&
    product.content === content &&
    product.images.length === images.length &&
    product.brand === brand &&
    product.price === price &&
    product.status === status &&
    product.oldPrice === oldPrice &&
    product.discount === discount &&
    product.stock === stock &&
    product.isFeatured === isFeatured &&
    product.model === model &&
    product.warranty === warranty &&
    product.weight === weight &&
    product.powerCategory === powerCategory &&
    product.productType === productType &&
    product.manufacturor === manufacturor &&
    product.performanceCoreFrequency === performanceCoreFrequency &&
    product.socket === socket &&
    product.coreCount === coreCount &&
    product.threadCount === threadCount &&
    product.l2Cache === l2Cache &&
    product.l3Cache === l3Cache &&
    product.benchmark === benchmark &&
    product.stockCooler === stockCooler;

  const areImagesSame = product.images
    .map((img) => img.publicId)
    .sort()
    .every(
      (publicId, index) =>
        publicId === images.map((img) => img.publicId).sort()[index]
    );

  if (isSameData && areImagesSame) {
    return next(new ApiError(400, "Nothing to update"));
  }

  // Delete old images from cloudinary
  if (!areImagesSame) {
    const deleteRemovedImagesFromCloudinary = async (oldImages, newImages) => {
      const imagesToDelete = oldImages.filter(
        (oldImage) =>
          !newImages.some((newImage) => newImage.publicId === oldImage.publicId)
      );

      for (let image of imagesToDelete) {
        const cloudinaryDeleteResult = await deleteImageFromCloudinary(
          image.publicId
        );
        if (image.publicId && !cloudinaryDeleteResult) {
          console.error(
            `Failed to delete image ${image.publicId} from Cloudinary`
          );
        }
      }
    };

    const oldImages = product.images || [];
    const newImages = images || [];

    await deleteRemovedImagesFromCloudinary(oldImages, newImages);
  }

  const updatedCpu = await Product.findByIdAndUpdate(
    productId,
    {
      name,
      description,
      content,
      images,
      brand,
      price,
      oldPrice,
      discount,
      stock,
      status,
      isFeatured,
      model,
      warranty,
      weight: weight ?? "",
      powerCategory,
      productType,
      manufacturor,
      performanceCoreFrequency,
      socket,
      coreCount,
      threadCount,
      l2Cache: l2Cache ?? 0,
      l3Cache: l3Cache ?? 0,
      benchmark,
      stockCooler,
    },
    { new: true }
  );

  if (!updatedCpu) {
    return next(new ApiError(500, "Something went wrong while updating CPU"));
  }

  res.status(200).json({
    success: true,
    message: "Product updated successfully",
    data: updatedCpu,
  });
});

export const RemoveCpuById = asyncHandler(async (req, res, next) => {
  const userData = req.user;

  if (userData.role !== "admin") {
    return next(new ApiError(403, "Unauthorized request"));
  }

  const { productId } = req.body;

  if (!productId) {
    return next(new ApiError(400, "Product id is required"));
  }

  const product = await Product.findById(productId);

  if (!product) {
    return next(new ApiError(404, "Product not found"));
  }

  // Delete images from cloudinary
  if (product?.images?.length > 0 && product.images[0]?.publicId) {
    for (const image of product.images) {
      if (image?.publicId) {
        const cloudinaryDeleteResult = await deleteImageFromCloudinary(
          image.publicId
        );
        if (!cloudinaryDeleteResult) {
          console.error(
            `Failed to delete image ${image.publicId} from Cloudinary`
          );
        }
      }
    }
  }

  const deletedProduct = await Product.findByIdAndDelete(productId);

  if (!deletedProduct) {
    return next(
      new ApiError(500, "Something went wrong while deleting product")
    );
  }

  return res.status(200).json({
    success: true,
    message: "Product deleted successfully",
  });
});

function validateCpu(data) {
  const schema = Joi.object({
    name: Joi.string().min(40).max(90).required(),
    description: Joi.string().min(40).max(1000).required(),
    content: Joi.string().min(40).max(10000).required(),
    images: Joi.array().min(1).required(),
    brand: Joi.string().required().min(3).max(30),
    model: Joi.string().required().min(3).max(40),
    price: Joi.number().min(0).required(),
    status: Joi.string().valid("active", "inactive").optional(),
    oldPrice: Joi.number().min(0).optional(),
    discount: Joi.number().min(0).max(100).required(),
    stock: Joi.number().min(0).required(),
    isFeatured: Joi.boolean().required(),
    warranty: Joi.string().required().min(3).max(20),
    weight: Joi.string().required().min(3).max(20),
    powerCategory: Joi.number().required().min(0).max(10),
    productType: Joi.string()
      .valid(
        "Cpu",
        "Ram",
        "Prebuild",
        "Gpu",
        "Cooler",
        "Psu",
        "Storage",
        "Case",
        "Motherboard"
      )
      .required(),
    performanceCoreFrequency: Joi.number().min(0.1).required(),
    manufacturor: Joi.string().required(),
    socket: Joi.string()
      .valid(
        "AM1",
        "AM2",
        "AM3",
        "AM4",
        "AM5",
        "LGA 775",
        "LGA 1150",
        "LGA 1151",
        "LGA 1155",
        "LGA 1156",
        "LGA 1200",
        "LGA 1366",
        "LGA 1700",
        "LGA 1851",
        "LGA 2011",
        "LGA 2066"
      )
      .required(),
    coreCount: Joi.number().min(1).max(64).required(),
    threadCount: Joi.number().min(1).required(),
    l2Cache: Joi.number().min(0.25).optional(),
    l3Cache: Joi.number().min(0).optional(),
    benchmark: Joi.number().min(0).required(),
    stockCooler: Joi.boolean().optional(),
  });
  return schema.validate(data);
}
